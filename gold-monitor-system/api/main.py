...
async def _start_calendar_sync() -> None:
    """Start the calendar sync background task (non-blocking).

    The initial sync runs as a background asyncio task so it does NOT
    block API startup.  The API becomes responsive immediately while
    the first calendar fetch happens in the background.
    """
    import asyncio as _aio

    async def _do_initial_sync() -> None:
        try:
    ...
    _aio.create_task(_do_initial_sync())
...
async def lifespan(_app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    try:
        ...
        logger.warning("Could not start calendar sync loop", exc_info=True)
